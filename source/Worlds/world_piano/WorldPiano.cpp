...
void WorldPiano::OnStart()
{
    ...
    SpriteRenderer* background = Primitive::CreateEmptyGameObject()->AddComponent<SpriteRenderer>();
...
void PianoPlayer::OnAwake()
{
    ...
    for (x = 0; x < NotesNum; ++x) {
        ...
        ResId note_wav = Resources::LoadAudioClip(std::string("./data/piano/") + notes[x].name + std::string(".wav"), true);
    ...
    spr_black = Primitive::CreateEmptySprite();
    spr_lf = Primitive::CreateEmptySprite();
    spr_cf = Primitive::CreateEmptySprite();
    spr_rf = Primitive::CreateEmptySprite();
...
#define set_layout(note) note.render->transform()->position(pos);
    notes[x = 0].key = KB_Q;
    notes[x].render->setSprite(spr_lf);
    ...
    x += 2;
    notes[x].key = KB_W;
    notes[x].render->setSprite(spr_cf);
    pos.x += scale.x;
    set_layout(notes[x]);

    x += 2;
    notes[x].key = KB_E;
    notes[x].render->setSprite(spr_rf);
    pos.x += scale.x;
    set_layout(notes[x]);
    ...
    // init key half-tone
    notes[x = 1].key = KB_2;
    notes[x].render->setSprite(spr_black);
