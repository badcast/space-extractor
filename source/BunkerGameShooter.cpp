...
void BunkerWorld::on_start()
{
    ...
    bulTempl->transform()->position(Vec2::infinity); // go inf (hide)
}
...
void BunkerWorld::on_update()
{
    ...
    turret->transform()->look_at_lerp(Camera::screen_to_world(ms), TimeEngine::deltaTime() * turret_rotate_speed);
...
void BunkerWorld::on_gizmo()
{
    ...
    // Vec2 direction = turret->transform()->forward();
    // Vec2 turretPos = turret->transform()->position();
    // Gizmos::draw_line_pivot(turretPos, direction);

    // Gizmos::draw_line_pivot(ms, Vec2::perpendicular(ms) );
}
