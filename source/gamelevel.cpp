...
std::vector<GameObject*> v(1000);
void fabricate()
{
    float t = 0;
    for (int x = 0; x < v.size(); ++x) {
        v[x] = instantiate(referenceObject);
        v[x]->transform()->position(Random::randomVector() * 3);
        destroy(v[x], t += 0.001f);
    }
}
uid slider;
void GameLevel::start()
{
    ...
    spr->setSprite(Primitive::CreateSprite2D({ 5, 5 }, Color::antiquewhite));

    slider = ui->Push_Slider(v.size(), 1.f, 10000.f, Vec2Int(0, 30));
...
void GameLevel::onDrawGizmos()
{
    ...
    Gizmos::DrawTextOnPosition(Camera::screen_2_world(Vec2::zero), std::string("Destroyed queue: ") + std::to_string(object_destruction_count()));
    Gizmos::DrawTextOnPosition(Camera::screen_2_world(Vec2::down * -20), std::string("Destroyed 1(frame): ") + std::to_string(v) + " destroyed 60(frames): " + std::to_string(n));

    auto res = Application::get_resolution();
    int time = TimeEngine::time();
    Gizmos::DrawTextOnPosition(Camera::screen_2_world(Vec2(10, res.height - 20)), (std::string((time / 60 < 10) ? "0" : "") + std::to_string(time / 60) + std::string(":") + std::string((time % 60 < 10) ? "0" : "") + std::to_string(time % 60)));
}
